...
public class MarvelPaths2 {
  ...
  public static @Nullable List<Graph.GraphEdge> MarvelPaths2(
      ...
      Queue<ArrayList<Graph.GraphEdge>> active =
          new PriorityQueue<ArrayList<Graph.GraphEdge>>(
              new Comparator<ArrayList<Graph.GraphEdge>>() {
                @Override
          ...
          for (String e : (Set<String>) minDest.getEdges().keySet()) {
            ...
            if (edge != null) {
              if (!finished.contains(edge.getDestination())) {
                double updatedWeight = (double) lastEdge.getLabel() + (double) edge.getLabel();
                //        newPath = minPath + e
                ArrayList<Graph.GraphEdge> newPath = new ArrayList<Graph.GraphEdge>(minPath);

                newPath.add(new Graph.GraphEdge(edge.getDestination(), (double) updatedWeight));
                active.add(newPath);
              }
            }
          }
