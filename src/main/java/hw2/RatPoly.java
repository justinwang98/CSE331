...
public final class RatPoly {
  ...
  public int degree() {
    if (terms.isEmpty()) {
      return 0;
    } else { // since terms is in ascending order assuming no 0 coeff, return first term in list
  ...
  public RatTerm getTerm(int deg) {
    ...
    return RatTerm.ZERO; // if no matching terms found, return 0
  }
  ...
  private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
    if (scalar.equals(RatNum.ZERO)) {
      ...
      for (int i = 0; i < lst.size(); i++) {
        RatTerm tempTerm = lst.get(i);
        lst.set(i, new RatTerm(tempTerm.getCoeff().mul(scalar), tempTerm.getExpt()));
      }
    }
  }
  ...
  private static void incremExpt(List<RatTerm> lst, int degree) {
    for (int i = 0; i < lst.size(); i++) {
      RatTerm tempTerm = lst.get(i);
      lst.set(i, new RatTerm(tempTerm.getCoeff(),tempTerm.getExpt() + degree));
      if (lst.get(i).getExpt() < 0) { // if degree is negative
        lst.remove(i);
        i--;
      }
    }
  ...
  public RatPoly negate() {
    if (this.isNaN()) {
      ...
      RatPoly poly2 = new RatPoly(terms);
      for (int i = 0; i < poly2.terms.size(); i++) {
        poly2.terms.set(i, poly2.terms.get(i).negate());
      }
      return poly2;
    }
  }
  ...
  public RatPoly add(RatPoly p) {
    if (this.isNaN() || p.isNaN()) { // if either this or p is NaN then return new NaN
      return NaN;
    } else  {
      RatPoly poly2 = new RatPoly(terms);
      for (RatTerm tp : p.terms) {
        sortedInsert(poly2.terms, tp);
      }
      return poly2;
    }
  }
  ...
  public RatPoly mul(RatPoly p) {
    if (this.isNaN() || p.isNaN()) {
      ...
      RatPoly poly2 = new RatPoly();
      for (RatTerm tp : p.terms) {
        for (RatTerm tp2 : this.terms) {
          sortedInsert(poly2.terms, new RatTerm(tp.getCoeff().mul(tp2.getCoeff()), tp.getExpt() + tp2.getExpt()));
        }
      }
      return poly2;
    }
   ...
   * @param p The divisor.
   * @spec.requires p != null
   * @return a RatPoly, q, such that q = "this / p"; if p = 0 or this.isNaN() or p.isNaN(), returns
   *     some q such that q.isNaN()
   *     <p>Division of polynomials is defined as follows: Given two polynomials u and v, with v !=
   *     "0", we can divide u by v to obtain a quotient polynomial q and a remainder polynomial r
   *     satisfying the condition u = "q * v + r", where the degree of r is strictly less than the
   *     degree of v, the degree of q is no greater than the degree of u, and r and q have no
   *     negative exponents.
   *     <p>For the purposes of this class, the operation "u / v" returns q as defined above.
   *     <p>The following are examples of div's behavior: "x^3-2*x+3" / "3*x^2" = "1/3*x" (with r =
   *     "-2*x+3"). "x^2+2*x+15 / 2*x^3" = "0" (with r = "x^2+2*x+15"). "x^3+x-1 / x+1 = x^2-x+2
   *     (with r = "-3").
  ...
  public RatPoly div(RatPoly p) {
    if (this.isNaN() || p.isNaN() || p.terms.size() == 0) {
      ...
      RatPoly polyResult = new RatPoly();
      RatPoly polyRemainder = new RatPoly(this.terms);
      while (polyRemainder.terms.size() != 0 && polyRemainder.degree() >= p.degree()) {
          RatPoly temp = new RatPoly(polyRemainder.terms.get(0).div(p.terms.get(0)));
          polyResult.add(temp);
          polyRemainder.sub(temp.mul(p));
      }
      polyResult.checkRep();
      return polyResult;
    }
  ...
  public RatPoly differentiate() {
    if (this.isNaN()) {
      ...
      RatPoly poly2 = new RatPoly();
      for (RatTerm tp : terms) {
        sortedInsert(poly2.terms, tp.differentiate());
      }
      return poly2;
    }
  }
  ...
  public double integrate(double lowerBound, double upperBound) {
    if ( this.isNaN() || lowerBound == Double.NaN || upperBound == Double.NaN) {
      ...
      RatPoly poly2 = new RatPoly(terms);
      RatPoly poly3 = new RatPoly(terms);
      poly2.antiDifferentiate(RatNum.ZERO);
      poly3.antiDifferentiate(RatNum.ZERO);
      return poly2.eval(upperBound) - poly3.eval(lowerBound);
    }
  }
  ...
  public double eval(double d) {
    if (this.isNaN()) {
      ...
      double total = 0.0;
      for (RatTerm tempTerm : terms) {
        total += tempTerm.eval(d);
      }
      return total;
    }
  }
   ...
   *     refers to a possible minus sign, if needed, and "(+|-)" refer to either a plus or minus
   *     sign, as needed. For each term, T takes the form "C*x^E" or "C*x" where {@code C > 0},
   *     UNLESS: (1) the exponent E is zero, in which case T takes the form "C", or (2) the
   *     coefficient C is one, in which case T takes the form "x^E" or "x". In cases were both (1)
   *     and (2) apply, (1) is used.
   ...
   *     form defined in the toString() method, except that the ordering of the terms by the degrees
   *     is not necessary.
