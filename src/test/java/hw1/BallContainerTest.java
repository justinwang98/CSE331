 ...
 * attempts to do so will fail, since the Ball is already in the BallContainer! Similarly, you
 * cannot expect to remove a Ball from a BallContainer if it is not inside the BallContainer.
 */
public class BallContainerTest {
  ...
  public void testAdd() {
    ...
    for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
      ...
      assertFalse(
          "BallContainer.add(Ball) seems to allow the same Ball to be added twice!",
  ...
  /** Test that BallContainer.iterator() is implemented correctly. */
  @Test
  ...
  public void testVolume() {
    ballcontainer.clear();
    assertEquals(
        "Volume of empty BallContainer is not zero!",
        0,
        ballcontainer.getVolume(),
        JUNIT_DOUBLE_DELTA);
    for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
  ...
  public void testSize() {
    ...
    for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
      ballcontainer.add(b[i]);
      assertEquals(
          "size() of BallContainer with " + (i + 1) + " ball(s)", i + 1, ballcontainer.size());
    }
  }
  ...
  public void testContains() {
    ...
    for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
      ballcontainer.add(b[i]);
      assertTrue(
          "BallContainer does not contain a Ball that is supposed to be inside!",
          ballcontainer.contains(b[i]));
      for (int j = i + 1; j < NUM_BALLS_TO_TEST; j++) {
        assertFalse(
            "BallContainer seems to contain a Ball that is not inside!",
            ballcontainer.contains(b[j]));
      }
    }
  }
